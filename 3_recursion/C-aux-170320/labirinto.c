...
int labirinto_percorribile(labirinto * lab, int r, int c) {
  ...
  labirinto_struct *l = (labirinto_struct*)lab;  
	...
	int perc = l->matrix[r][c] == LABIRINTO_VUOTA && l->marcata[r][c] == LABIRINTO_FALSE; 
	return perc;
}
...
int labirinto_uscitaraggiungibileda(labirinto * lab, int r, int c) {
  ...
  labirinto_struct *l = (labirinto_struct*)lab;  
	...
	l->marcata[r][c] = LABIRINTO_TRUE; //giusto
	if(labirinto_uscita(lab, r, c))	return 1;
	
	
	if(labirinto_percorribile(lab, r, c+1)){ //if =>else if
		//~ x = r;
		//~ y = c+1;
		if(a) return 1;
		a |= labirinto_uscitaraggiungibileda(lab, r, c+1);
	}	
	if(labirinto_percorribile(lab, r+1, c)){
		//~ x = r+1;
		//~ y = c;
		if(a) return 1;
		a |= labirinto_uscitaraggiungibileda(lab, r+1, c);
	}
	if(labirinto_percorribile(lab, r, c-1)){
		//~ x = r;
		//~ y = c-1;
		if(a) return 1;
		a |= labirinto_uscitaraggiungibileda(lab, r, c-1);
	...
	if(!a) l->marcata[r][c] = LABIRINTO_FALSE; //if(a) segna solo i blocchi
	//if(a) l->marcata[r][c] = LABIRINTO_TRUE;
