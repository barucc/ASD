...
void linked_list_remove(linked_list *ll, void* elem){
	linked_list_node * ptr = ll->head;
	if(!ptr){
		return;
	}
	if(ptr->value == elem){
		...
		ll->size--;
		ptr = ll->head;
		while(ll->head && ptr->next){
			ptr = ptr->next;
		}
		ll->tail = ptr;
		return;
	}
	while(ptr->next && ptr->next->value != elem){
		ptr = ptr->next;
	}
	if(!ptr->next)
		return;
	if(ptr->next->value == elem){
		ll->size--;
